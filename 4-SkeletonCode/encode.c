...
Status encode_secret_file_data(EncodeInfo *encInfo)
{
    ...
    fread(buffer, sizeof(char[encInfo->size_secret_file + 1]), 1,encInfo->fptr_secret);
...
Status encode_integer(int size, char *image_buffer)
{
    for(int i=0; i<32; i++)
    {
        uint get_bit = (uint)(size & (1 << i)) >> (i); //get each byte from 1 byte of integer & move the bit to lsb
        uint clear_bit = image_buffer[i] & (~1);  // clear lsb bit of source file data in 1 byte
...
Status encode_character(char data, char *image_buffer) 
{
    for(int i=0; i<8; i++)
    {
        uint get_bit = (uint)(data & (1 << i)) >> (i); //get each byte from 1 byte of character & move the bit to lsb
        uint clear_bit = image_buffer[i] & (~1); // clear lsb bit of source file data in 1 byte
        image_buffer[i] = get_bit | clear_bit; 
    }
    return e_success;
